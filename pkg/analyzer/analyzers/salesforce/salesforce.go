//go:generate generate_permissions permissions.yaml permissions.go salesforce
...
func (a Analyzer) Type() analyzers.AnalyzerType {
	return analyzers.AnalyzerTypeSalesforce
}
...
func AnalyzeAndPrintPermissions(cfg *config.Config, domain, token string) {
	info, err := AnalyzePermissions(cfg, token, domain)
	if err != nil {
		// just print the error in cli and continue as a partial success
		color.Red("[x] Error : %s", err.Error())
	} else {
		color.Green("[!] Valid Salesforce API token\n\n")
	}
...
func AnalyzePermissions(cfg *config.Config, domain, token string) (*SecretInfo, error) {
	...
	body, _, err := capturePermissions(client, domain, token)
	if err != nil {
		return secretInfo, fmt.Errorf("failed to check permissions: %w", err)
	}

	var permissionsResp SalesforcePermissionsResponse
	...
	if err := captureResources(client, domain, token, secretInfo, grantedPermissions); err != nil {
...
func secretInfoToAnalyzerResult(info *SecretInfo) *analyzers.AnalyzerResult {
	...
	result := analyzers.AnalyzerResult{
		AnalyzerType: analyzers.AnalyzerTypeSalesforce,
...
func secretInfoResourceToAnalyzerResource(resource SalesforceResource) *analyzers.Resource {
...
// cli print functions
func printUserInfo(user SalesforceUser) {
	if user.AccountID == "" {
...
func printResources(resources []SalesforceResource) {
	if len(resources) == 0 {
